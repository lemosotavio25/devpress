                ...
                <div class="inline-block align-bottom bg-white dark:bg-gray-800 rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
                                ...
                                <div class="mt-2">
                                    ...
                                    @if($user?->articles_count > 0)
                                        <p class="mt-2 text-sm text-yellow-600 dark:text-yellow-400 font-medium">
                                            ⚠️ Este usuário possui {{ $user->articles_count }} artigo(s) criado(s).
                                        </p>
                                    @endif
                                    @if($user?->contributed_articles_count > 0)
                                        <p class="mt-2 text-sm text-yellow-600 dark:text-yellow-400 font-medium">
                                            ⚠️ Este usuário é contribuidor em {{ $user->contributed_articles_count }} artigo(s).
                                        </p>
                                    @endif
                    ...
                    <div class="bg-gray-50 dark:bg-gray-700 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse gap-2">
                        ...
                        <button 
                            type="button" 
                            wire:click="closeModal"
                            class="mt-3 w-full inline-flex justify-center rounded-lg border border-gray-300 dark:border-gray-600 shadow-sm px-4 py-2 bg-white dark:bg-gray-800 text-base font-medium text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-purple-500 sm:mt-0 sm:ml-3 sm:w-auto sm:text-sm transition duration-150"
                        >
                            Cancelar
                        </button>
                    </div>
